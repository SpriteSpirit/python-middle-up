...
- Здесь программа разделена на несколько потоков, которые могут выполняться параллельно в рамках одного процесса.
- В Python используется модуль threading.
- Потоки делят между собой один и тот же адресное пространство (память), что может привести к проблемам с синхронизацией доступа к данным, если не использовать механизмы блокировки (локи).
- Из-за GIL (Global Interpreter Lock) в Python многопоточность не всегда приводит к реальному параллелизму на уровне CPU, особенно для CPU-интенсивных задач.
